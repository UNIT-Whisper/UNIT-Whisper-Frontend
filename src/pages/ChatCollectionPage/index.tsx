...
function ChatCollectionPage() {
  ...
  return <div className="relative w-full h-[830px] overflow-y-scroll overflow-x-hidden">
              {clouds.map((el,index) => (
...
const Cloud = (prop : cloudProp) => {
  ...
  const [ addCloud, removeCloud] = useCheckCloudStore((state) => [state.addCloud, state.removeCloud]);
...
>
  <div
  className="text-center text-base font-bold text-[##4A5568] w-40 font-Jalnan"
  >
    {prop.text}
  </div>
</div>
