...
const MarkerInfo: Array<stampType> = [
  {
    latitude: 37.547282739995964,
    longitude: 127.06659525293044,
    content: '안녕',
    createDate: new Date(),
    address: '서울특별시 성동구 아차산로 17길 48',
    whisperId: 1,
  },
];
...
const MarkInfo = ({ info }: { info: stampType }) => {
  const InfoBtn = useCallback(({ children }: { children: ReactNode }) => {
    return (
      <div className="flex items-center justify-center gap-1.5 rounded-md bg-[#EDF2F7] px-2 py-1.5 font-Pretendard text-xs font-semibold text-[#4A5568]">
        {children}
      </div>
    );
  }, []);
  return (
    <div
      className="absolute bottom-0 z-50 flex h-[350px] w-full flex-col items-start gap-5 bg-white px-6 py-9"
      style={{
        borderRadius: '20px 20px 0px 0px',
      }}
    >
      <div className="w-full border-b-[1px] border-solid border-b-[#E2E8F0] py-4">
        <img src={cloudMarker} alt="정보로고" className="h-12 w-[70px]" />
