...
const MapPage = () => {
  ...
  const [myposition, setmyPosition] = usePositionstore((state) => [
    state.position,
    state.setPosition,
  ]);
  const [clouds, resetCheckCloud] = useCheckCloudStore((state) => [
    state.clouds,
    state.resetClouds,
  ]);
  ...
  const handleChangeMarker = async () => {
    ...
    const response = await sendCloud(
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div
        ...
        className="absolute bottom-0 z-10 flex h-[329px] w-full max-w-[500px] flex-col items-center justify-center"
      >
        <img
          src={cloudBtn}
          alt="구름 버튼"
          className="absolute left-[35%] top-[5%] h-[115px] w-[164px]"
        />
        <div className="mb-2 mt-8 text-center font-Jalnan text-[28px] font-bold text-[#2D3748]">
          속삭임
        </div>
        <button
          className="inline-flex items-center justify-center gap-1 rounded p-3 font-Pretendard font-semibold text-white"
          style={{
